...
OnInit.module("Hook", function()
    ...
    function Hook.add(key, callback, priority, host, default, metatable, basic)
        ...
        if tree then
            local exit = #tree
            repeat
                if priority <= tree[index].priority then break end
                index = index + 1
            until index > exit
        else
            tree = { host = host, key = key,
                [0] = rawget(host, key) or default or (host ~= _G or type(key) ~= "string") and DoNothing or (Debug and Debug.error or print)("No value found for key: " .. key) }
            treeMap[host][key] = tree
        end
        local new = { priority = priority, tree = tree, basic = callback }
        if not basic then
            setmetatable(new, { __call = callback, __index = getIndex })--[[@as Hook.property]].basic = false
        end
    ...
    ---@deprecated
    function AddHook(...)
        local new = Hook.basic(...); return function(...) return new.next(...) end, new.remove
    end
