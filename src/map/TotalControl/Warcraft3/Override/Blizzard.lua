...
--  Blizzard.j ( re-define Jass2 functions that need to be in every map script )
...
function AngleBetweenPoints(locA, locB)
	return math.deg(Atan2(GetLocationY(locB) - GetLocationY(locA), GetLocationX(locB) - GetLocationX(locA)))
end
...
function PolarProjectionBJ(source, dist, angle)
	local x = GetLocationX(source) + dist * Cos(math.rad(angle))
	local y = GetLocationY(source) + dist * Sin(math.rad(angle))
	return Location(x, y)
end
...
function IntegerTertiaryOp(flag, valueA, valueB)
    return flag and valueA or valueB
end
-- ***************************************************************************
--
